...
describe OauthsController do
  ...
  describe "GET 'callback'" do
    it "creates a new user" do
      allow(controller).to receive(:login_from).with('github').and_return(nil) # no authentication
      user = User.new(username: 'RockyII', name: 'Rocky Balboa')
      allow(controller).to receive(:create_from).with('github').and_return(user)
    ...
    it "logs in a known user" do
      ...
      allow(controller).to receive(:login_from).with('github').and_return(user)
